...
hook.Add("PlayerButtonDown", "am_KeyDetection", function(ply, btn)
  ...
  if IsValid(wep) and table.HasValue(antManSuits, wep:GetClass()) then
    if btn == KEY_J then
      if ply:GetModelScale() != 1 then
        ply:am_SetModelScale(1, 0.2)
      else
  ...
  if IsValid(wep) and table.HasValue(antManWeapons, wep:GetClass()) then
